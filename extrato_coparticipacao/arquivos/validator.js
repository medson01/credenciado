...
a_messages = [
	...
	'O valor para "%l%" deve ter %mn% caracteres ou mais.',
	'O valor para "%l%" deve ser menor que %mx% caracteres.',
	'"%v%" não é válido para "%l%". Não usar acentuação.',
	'"%l%" deve ser do tipo "%ml%"'
]
...
function validator_exec() {
	...
	if (b_dom)
		for (var n_key in this.a_fields) 
			if (this.a_fields[n_key]['t']) {
				...
				// alert(e_labeltag.className);
				$("#" + o_form.elements[n_key].name + "-error").slideUp(500);
			}
	...
	if (n_errors_count) {
		...
		// alert(s_alert_message);		// validação original do componente.
		$("#" +  e_first_error.name +"-error").slideDown(200);
