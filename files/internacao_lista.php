...
*/
  ...
  If( $_SESSION["perfil"] == "usuario" || $_SESSION["perfil"] == "alimentacao"){
    ...
    if(isset($_GET['buscar'])){
       ...
       $b = " WHERE usuarios.id_credenciado = '".$_SESSION["id_credenciado"]."' and (internamento.nome like '%".$_GET['buscar']."%' or internamento.id = '".$_GET['buscar']."' or internamento.matricula = '".$_GET['buscar']."') order by internamento.id DESC";
    }elseif(isset($_GET['mes'])){

        $b = " WHERE usuarios.id_credenciado = '".$_SESSION["id_credenciado"]."' and MONTH(internamento.dat_entrada) = ".$mes." and Year(internamento.dat_entrada) = '".$ano."' order by internamento.id DESC";
    }else{


      // Consulta de entrada Dedault para usuário 
       $b = "  WHERE usuarios.id_credenciado = '".$_SESSION["id_credenciado"]."' and (internamento.dat_saida is null OR internamento.dat_saida = '0000-00-00 00:00:00')  order by internamento.id DESC";
    }
    ...
    $a = $a.$b;
        ...
        if(isset($_GET['buscar'])){
          ...
          $b = " WHERE  (internamento.nome like '%".$_GET['buscar']."%' or credenciado.nome like '%".$_GET['buscar']."%' or internamento.id = '".$_GET['buscar']."' or internamento.matricula = '".$_GET['buscar']."' or usuarios.nome like '%".$_GET['buscar']."%') order by  credenciado.nome, internamento.id DESC";

         
        }elseif(isset($_GET['mes'])){

          $b= " WHERE MONTH(internamento.dat_entrada) = ".$mes." and Year(internamento.dat_entrada) = ".$ano." order by  credenciado.nome ,internamento.id DESC";

        }else{

           $b= " WHERE  (internamento.internacao_numero_totvs = '' or internamento.internacao_numero_totvs is null) AND (internamento.dat_saida is null OR internamento.dat_saida = '0000-00-00 00:00:00') order by credenciado.nome,internamento.id  DESC ";
      ...
      $a = $a.$b;
        ...
        if(isset($_GET['buscar'])){
          ...
          $b = " WHERE (internamento.nome like '%".$_GET['buscar']."%' or credenciado.nome like '%".$_GET['buscar']."%' or internamento.id = '".$_GET['buscar']."' or internamento.matricula = '".$_GET['buscar']."' or usuarios.nome like '%".$_GET['buscar']."%') order by  internamento.id  DESC";

         
        }elseif(isset($_GET['mes'])){

          $b= " WHERE MONTH(internamento.dat_entrada) = ".$mes." and Year(internamento.dat_entrada) = ".$ano." order by internamento.id  DESC";

        }else{

           $b= " WHERE internamento.dat_saida is null OR internamento.dat_saida = '0000-00-00 00:00:00' order by internamento.id  DESC";
               ...
               <tr  style='font-weight:bold;'>
                ...
                <!-- <td style='padding: 4px;'><div align="center">Previsão</div></td>  -->
                 ...
                 <?php If( $_SESSION["perfil"] == "administrador" or $_SESSION["perfil"] == "auditor" or $_SESSION["perfil"] == "medico"){ echo "<td style='padding: 4px;'><div align='center'>Credenciado</div></td>"; } ?>            
				   ...
				   <td style='padding: 4px;'><div align="center">Alimentação</div></td> 
              ...
              <?php
                ...
                error_reporting(E_ALL ^ E_NOTICE);
                 ...
                 // Campo PA - Pronto atendimento
                  ...
                  // Campo Status - Alimentação
                        echo "<td><div align='center' style='color: #FF4000; font-weight: bold;'> <font color='#FF4000'>";
                        
                          if($registro["status_alimentacao"] == 1){ 
                            echo "<font ><strong><a  style='font-family: Andale monospace;'  ".$cor." href=\"javascript:func()\" onmouseover=\"Tip(' Alimentação em análise ')\" onmouseout=\"UnTip()\"> ";
                            echo '<img src="../imagem/alimentacao01.png" width="20" height="20" style="margin:0">';
                          }
                  ...
                  // Campo Autorização Internação        
                          if(!empty($registro["internacao_numero_totvs"])){ 
                            echo "<td align='center'><font ><strong><a  style='font-family: Andale monospace;'  ".$cor." href=\"javascript:func()\" onmouseover=\"Tip('Nº: ".$registro["internacao_numero_totvs"]." ')\" onmouseout=\"UnTip()\"> ";
                  ...
                  //Botão Saída   
                       ...
                       if (  (empty($registro["dat_saida"])) && ($_SESSION["perfil"] <> "aut_internacao") && ( $_SESSION["perfil"] <> "alimentacao") && ( $_SESSION["perfil"] <> "medico")){                
                        echo " 
                                    <!-- Botão sair 
